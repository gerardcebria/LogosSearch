...
def query_from_text(client: WeaviateClient, query_text: str, the_limit: int = 3):
    ...
    response = (client.client.query
                   ...
                   .do())
    if response["data"]["Get"]["Logos"]:
        found_picts = response["data"]["Get"]["Logos"]
...
def reload_data(client: WeaviateClient) -> None:
    """
    Loads a fresh instance of the Logos schema, removes the old one if present.
    :param client: Client for interacting with Weaviate
    :return: Nothing
    """
    load_weaviate_schema(client=client, schema_path="./config_files/weaviate_logos.json")
    store_images(client)
...
if __name__ == '__main__':
    ...
    the_query = st.text_input('describe the logo in one or a few words')
